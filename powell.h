...
vector<array<double,7> > conjugate(Polys& p,double x,double y,double x_max,double x_min,double y_max,double y_min,double tau){
    //record
    vector<array<double,7> > record;
    array<double,2> g = p.gradient(x,y),g2,s;
    double x2,y2,beta=0,a,dif;
    int i=0;
    s[0] = -g[0];
    s[1] = -g[1];
    ...
    do{
        ...
        g2 = p.gradient(x2,y2);
        //beta
        beta = (g2[0]*g2[0]+g2[1]*g2[1])/(g[0]*g[0]+g[1]*g[1]);
        s[0] = -g2[0] + beta*s[0];
        s[1] = -g2[1] + beta*s[1];
        dif = x2-x;
        x = x2;
...
vector<array<double,6> > newton(Polys& p,double x,double y,double x_max,double x_min,double y_max,double y_min,double tau){
    ...
    do{
        ...
        hi[1][1] = h[0][0]/(h[0][0]*h[1][1]-h[1][0]*h[0][1]);
